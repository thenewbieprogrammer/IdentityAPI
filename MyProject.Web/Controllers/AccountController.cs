...
using Identity.Web.Models.SystemViewModels;
...
namespace Identity.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        [AllowAnonymous]
        //[ValidateAntiForgeryToken]
        public async Task<bool> LoginLite(LoginViewModel model)
        {
            if (!ModelState.IsValid)
            {
               // return View(model);
        ...
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            {
                AddViewMessage(StandardMessages.CustomMessageError, "Invalid confirmation link. Please request the confirmation link again.");
                return View();
            }
            ...
            try
            {
                ...
                var userRecord = _userService.GetUser(id);
                if (userRecord == null)
                {
                    AddViewMessage(StandardMessages.CustomMessageError, "Invalid confirmation link. Please request the confirmation link again.");
                }
                else
                {
                    ...
                    {
                        AddViewMessage(StandardMessages.CustomMessageInfo, "Email is already confirmed!");
                        return RedirectToAction("Index", "Home");
                    }
                    var result = await UserManager.ConfirmEmailAsync(userId, code);
                    if (result == IdentityResult.Success)
                    {
                        AddViewMessage(StandardMessages.CustomMessageInfo, "Email has been confirmed!");
                    }
                    else
                    {
                        AddViewMessage(StandardMessages.CustomMessageError, "Faild to confirm email address!");
                    }
                }
            }
            catch (Exception)
            {
                AddViewMessage(StandardMessages.CustomMessageError, "Error confirming email address. Please try again later!");
            }
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                case SignInStatus.Failure:
                    AddViewMessage(StandardMessages.CustomMessageError, "Loging in.");
                    break;
                default:
        ...
        public ActionResult LogOff()
        {
            ...
            AddViewMessage(StandardMessages.CustomMessageSuccess, "You have been logged out.");
            return RedirectToAction("Index", "Home");
        }
